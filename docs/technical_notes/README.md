...
Standalone, citation-quality writeups of the algorithms and data
products that ship with the PRad-II event-viewer / replay code.  Each
note is self-contained: it covers the algorithm, the C++ entry point,
the relevant configuration knobs, an example trace, and a NumPy /
Matplotlib reference implementation that regenerates every figure.
...
└── scripts/            # NumPy + Matplotlib reference implementation
                        # that regenerates the figures and prints the
...
| 2 | [HyCal Clustering](hycal_clustering/hycal_clustering.md) | Chao Peng (ANL) | `fdec::HyCalCluster` Island clustering: DFS grouping, log-weighted centre of gravity, profile-based shower split, shower-depth correction. |
...
adding a new note, list the author and affiliation there as well as in
the table above.

For the firmware-mode algorithm specification (with full FADC250
manual cross-references), see also
...
NumPy + Matplotlib only.  From the note's directory:
...
The HyCal and GEM scripts also read `database/hycal_map.json` and
`database/gem_map.json` to draw the real geometry, so they must be
run from a checkout of this repository (the relative paths walk up to
the repository root).
...
or release tag so the reader can check out the exact version
referenced.

Use the author(s) listed in the **Author** column above (also at the
top of each note) — *not* "PRad-II Collaboration" — since technical
...
**Worked example** (waveform analysis; replace `<sha>` and `<YYYY-MM-DD>`
...
For lab notebooks where pinning to a commit is unimportant, a
permalink to the note on `main` suffices:
